...
![Мониторинг работы почтовой службы (2)](image/лаба10_12.png){#fig-012 width=50%}
...
![Редактирование файла /etc/dovecot/conf.d/10-master.conf (2)](image/лаба10_14.png){#fig-014 width=60%}
...
Настроили Postfix для приёма почты из Интернета только для обслуживаемых нашим сервером пользователей или для произвольных пользователей локальной машины, обеспечивая тем самым запрет на использование почтового сервера в качестве SMTP relay для спамрассылок ([рис. @fig-016])
...
Для проверки работы аутентификации временно запустим SMTP-сервер (порт 25) с возможностью аутентификации. Для этого в файле */etc/postfix/master.cf* заменили строку ```smtp inet n - n - - smtpd``` на строку ([рис. @fig-018]):
...
![Подключение к SMTP-серверу посредством telnet](image/лаба10_22.png){#fig-022 width=35%}
...
![Настройка межсетевого экрана, разрешение рабоать службе smtp-submission](image/лаба10_26.png){#fig-026 width=50%}
...
![Тестирование подключения по telnet](image/лаба10_29.png){#fig-029 width=50%}
...
![Настройки учётной записи](image/лаба10_31.png){#fig-031 width=40%}
...
![Отправка почтового сообщения](image/лаба10_32.png){#fig-032 width=50%}
...
![Редактирование файла mail.sh на сервере](image/лаба10_36.png){#fig-036 width=35%}
